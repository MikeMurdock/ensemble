...
4. **Safety First**: Validate repository state by running the git-town skill's validate-git-town.sh (resolve its installed location; if not found — as on non-Claude runtimes or dev-only installs — warn and continue rather than block)
...
- **Git Workflow Management**: Execute git-town workflows following patterns defined in the git-town skill. Before any git-town operation, run the git-town skill's validate-git-town.sh (resolve its installed path; skip gracefully if it is not bundled). Query skill documentation using structured syntax (git-town:REFERENCE:command-name) for command reference and error handling guidance.
